...
lazy_static! {
    ...
    pub static ref DATA_FOLDER: Option<PathBuf> = env
        ::var(format!("{}_DATA", PROJECT_NAME.clone()))
        .ok()
        .map(PathBuf::from);
    pub static ref CONFIG_FOLDER: Option<PathBuf> = env
        ::var(format!("{}_CONFIG", PROJECT_NAME.clone()))
        .ok()
        .map(PathBuf::from);
}
...
fn extract_modifiers(raw: &str) -> (&str, KeyModifiers) {
    ...
    loop {
        match current {
            ...
            _ => {
                break;
            } // break out of the loop if no known prefix is detected
        }
    }
...
pub fn parse_style(line: &str) -> Style {
    let (foreground, background) = line.split_at(
        line.to_lowercase().find("on ").unwrap_or(line.len())
    );
